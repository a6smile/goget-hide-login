<?php
/*
Plugin Name: GoGet Hide Login
Plugin URI: https://github.com/a6smile/goget-hide-login
Description: Change your login url and remove access to wp-login.php page 
Author: Anom Wijaya
Author URI: http://a6smile.com
Version: 1.1.2
Text Domain: goget-hide-login
License: GPLv2 or later
*/

if ( defined( 'ABSPATH' )
	&& ! class_exists( 'GoGet_Hide_Login' ) ) {

	class GoGet_Hide_Login {
		...
		public function admin_notices_incompatible() {
			...
			echo '<div class="error notice is-dismissible"><p>' . __( 'Please upgrade to the latest version of WordPress to activate', 'goget-hide-login') . ' <strong>' . __( 'GoGet Hide Login', 'goget-hide-login') . '</strong>.</p></div>';

		}

        public function admin_notices_plugin_conflict() {

			echo '<div class="error notice is-dismissible"><p>' . __( 'GoGet Hide Login could not be activated because you already have Rename wp-login.php active. Please uninstall rename wp-login.php to use GoGet Hide Login', 'goget-hide-login') . '</p></div>';
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'GoGet Hide Login', 'goget-hide-login') . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'goget-hide-login' ) . '</p>';
			$out .= '<p>' . sprintf( __( 'Need help? Try the <a href="%s" target="_blank">visit our website</a>.', 'goget-hide-login' ), 'http://a6smile.com' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row"><label for="whl_page">' . __( 'Networkwide default', 'goget-hide-login' ) . '</label></th>';
		...
		public function admin_init() {
			...
			add_settings_field(
				'whl_page',
				'<label for="whl_page">' . __( 'Login url', 'goget-hide-login' ) . '</label>',
		...
		public function whl_section_desc() {
			...
			if ( is_multisite()
				...
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to <a href="%s">Network Settings</a>.', 'goget-hide-login' ), network_admin_url( 'settings.php#whl-page-input' ) ) . '</p>';
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin()
				...
				echo '<div class="updated notice is-dismissible"><p>' . sprintf( __( 'Your login page is now here: <strong><a href="%1$s">%2$s</a></strong>. Bookmark this page!', 'goget-hide-login' ), $this->new_login_url(), $this->new_login_url() ) . '</p></div>';
		...
		public function plugin_action_links( $links ) {
			...
			if ( is_network_admin()
				...
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#whl-page-input' ) . '">' . __( 'Settings', 'goget-hide-login' ) . '</a>' );

			} elseif ( ! is_network_admin() ) {

				array_unshift( $links, '<a href="' . admin_url( 'options-general.php#whl-page-input' ) . '">' . __( 'Settings', 'goget-hide-login' ) . '</a>' );
		...
		public function plugins_loaded() {
			...
			if ( ! is_multisite()
				...
				wp_die( __( 'This feature is not enabled.', 'goget-hide-login' ) );
        ...
        public function whl_load_textdomain() {
            load_plugin_textdomain( 'goget-hide-login', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
        }

	}

	add_action( 'plugins_loaded', array( 'GoGet_Hide_Login', 'get_instance' ), 1 );
}